...
namespace Mismo.Gameplay.Player.Equipment.Inventory
{
    public sealed partial class InventoryPanel
    {
        ...
        void DrawQuietInterface()
        {
            ...
            if(page==Page.Bestiary||page==Page.Mounts)
            {
                var sectionIcon=page==Page.Mounts?inventoryIcons?.radialMounts:inventoryIcons?.radialBestiary;
                var color=GUI.color;GUI.color=NavigationTint(U.Ink);
                if(sectionIcon!=null)GUI.DrawTexture(new Rect(70,44,34,34),MapIcons.Mask(sectionIcon),ScaleMode.ScaleToFit);
                else U.DrawIcon(new Rect(70,44,34,34),page==Page.Mounts?"wingfoot":"wolf-trap",NavigationTint(U.Ink));
                GUI.color=color;GUI.Label(new Rect(65,38,48,47),new GUIContent("",L.Text(page==Page.Mounts?"Monturas":"Bestiario")),GUIStyle.none);
            }
            else
            {
            if(InventoryIconButton(new Rect(480,38,88,47),inventoryIcons?.radialCharacter,"Personaje",page==Page.Character))OpenPage(Page.Character);
            if(InventoryIconButton(new Rect(592,38,88,47),inventoryIcons?.weapons,"Armas",page==Page.Weapons))OpenPage(Page.Weapons);
            if(InventoryIconButton(new Rect(704,38,88,47),inventoryIcons?.radialSkills,"Habilidades",page==Page.Skills))OpenPage(Page.Skills);
            }
            QuietLine(64,104,1150);
            if(!IsOpen||!IsQuietPage)return;
            ...
            else if(page==Page.Bestiary)bestiary.Draw(inventory,preview,inventoryIcons,DrawPreview);
            else DrawMounts();
            if(inventory.HasSaveProblem)U.Text(new Rect(65,737,1140,30),inventory.Notice,16,new Color(1,.6f,.3f));
            else if(!string.IsNullOrEmpty(GUI.tooltip))U.Text(new Rect(65,737,1140,30),GUI.tooltip,16,U.Muted);
        }
