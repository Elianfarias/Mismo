...
namespace Mismo.Gameplay.Player.Equipment.Inventory
{
    public sealed class BestiaryView
    {
        ...
        public void Draw(PlayerInventory inventory,InventoryPreview preview)
        {
            ...
            if(pages.Length==0){Presentation.PlayerHUD.Fill(new Rect(120,130,1030,510),book.paper);var empty=new GUIStyle(GUI.skin.label){fontSize=26,wordWrap=true};empty.normal.textColor=book.ink;GUI.Label(new Rect(200,280,860,150),L.Text("Todavía no descubriste ninguna criatura. Explorá el mundo para completar el bestiario."),empty);return;}
            ...
            var label=new GUIStyle(GUI.skin.label){fontSize=23,wordWrap=true};label.normal.textColor=book.ink;
