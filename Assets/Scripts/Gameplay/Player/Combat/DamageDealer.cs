...
namespace Mismo.Gameplay.Combat
{
    ...
    public sealed class DamageDealer : MonoBehaviour
    {
        ...
        private readonly HashSet<EntityId> hitObjects = new HashSet<EntityId>();
        ...
        {
            if (!applyOnTriggerEnter || !hitObjects.Add(other.transform.root.GetEntityId())) return;
            Vector3 direction = other.transform.position - transform.position;
            ApplyTo(other.gameObject, other.ClosestPoint(transform.position), direction);
        }

        private void OnTriggerExit(Collider other) => hitObjects.Remove(other.transform.root.GetEntityId());
    }
}
