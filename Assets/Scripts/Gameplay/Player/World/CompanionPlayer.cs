...
namespace Mismo.Gameplay.Player.World
{
    ...
    public sealed class CompanionPlayer:MonoBehaviour
    {
        ...
        Animator mountAnimator;bool hasMotion;
        ...
        void Update()
        {
            ...
            if(Riding)
            {
                ...
                if(hasMotion)mountAnimator.SetInteger("Motion",velocity.sqrMagnitude>.1f?2:0);
                return;
            }
            ...
            if(hasMotion)mountAnimator.SetInteger("Motion",agent!=null&&agent.enabled&&agent.velocity.sqrMagnitude>.1f?1:0);
        }
        IEnumerator Spawn()
        {
            loading=true;species=CreatureSpecies.Find(inventory.CompanionSpeciesId);
            ...
            var model=new GameObject(prefab.name);var visual=CloneVisual(prefab);visual.transform.SetParent(model.transform,false);visual.transform.localPosition=Vector3.zero;
            yield return null;
            companion=model;model.name="Companion - "+species.displayName;model.transform.position=hit.position;
        ...
        public static GameObject CloneVisual(GameObject prefab)
        {
            ...
            foreach(var original in prefab.GetComponentsInChildren<Renderer>(true))
            {
                ...
                if(original is SkinnedMeshRenderer skin){var copy=target.AddComponent<SkinnedMeshRenderer>();copy.sharedMesh=skin.sharedMesh;copy.sharedMaterials=skin.sharedMaterials;copy.bones=System.Array.ConvertAll(skin.bones,b=>b!=null&&mapping.ContainsKey(b)?mapping[b]:null);copy.rootBone=skin.rootBone!=null?mapping[skin.rootBone]:null;copy.localBounds=skin.localBounds;}
            ...
            foreach(var original in prefab.GetComponentsInChildren<Animator>(true)){var copy=mapping[original.transform].gameObject.AddComponent<Animator>();copy.avatar=original.avatar;copy.runtimeAnimatorController=original.runtimeAnimatorController;copy.applyRootMotion=false;}
            return root;
        }
        ...
        void Mount()
        {
            if(species==null||!species.mountable||species.riderPose==null)return;
        ...
        void LateUpdate()
        {
            if(!Riding||companion==null)return;
            var pose=species.riderPose;transform.SetPositionAndRotation(companion.transform.TransformPoint(pose.position),companion.transform.rotation*Quaternion.Euler(pose.rotation));
        ...
        void Dismount()
        {
            ...
            Riding=false;if(ridingTarget!=null)ridingTarget.enabled=false;mountBody.enabled=false;agent.enabled=true;if(agent.isOnNavMesh)agent.Warp(companion.transform.position);
            if(motor.Visual!=null){motor.Visual.localPosition=visualPosition;motor.Visual.localRotation=visualRotation;motor.Visual.localScale=visualScale;}
            if(animator!=null){animator.enabled=true;animator.Rebind();}if(animationDriver!=null)animationDriver.enabled=true;
            motor.ResetPosition(destination);controller.enabled=true;notice=null;
        }
        void OnGUI()
        {
        ...
        void OnDestroy(){if(companion!=null)Destroy(companion);}
    }
}
