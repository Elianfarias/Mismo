...
namespace Mismo.Gameplay.Player.Editor
{
    public static class CraftingExpansionAssets
    {
        ...
        public static void Create()
        {
            ...
            if(nodes.Length<3)
            {
                nodes=Resources.LoadAll<ResourceNodeDefinition>("Gathering/Minerals").OrderBy(n=>n.name,StringComparer.Ordinal).ToArray();
            ...
            var potion=Ensure<MaterialDefinition>("Assets/Resources/Materials/HealthPotion.asset",m=>{
            ...
            var whetstone=Ensure<MaterialDefinition>("Assets/Resources/Materials/Whetstone.asset",m=>{
            ...
            var herb=Resources.Load<MaterialDefinition>("Materials/Herb");var wood=Resources.Load<MaterialDefinition>("Materials/Wood");
            var stone=Resources.Load<MaterialDefinition>("Materials/Stone");var monster=Resources.Load<MaterialDefinition>("Materials/MonsterComponent");
        ...
        {
            var r=Ensure<CraftingRecipe>("Assets/Resources/Recipes/"+name+".asset",x=>{x.id=id;x.displayName=title;x.description=description;x.ingredients=cost;setup(x);});
            if(!list.Contains(r))list.Add(r);
        }
        static MaterialDefinition Material(string name,string id,string title,string description,GameObject prefab)=>Ensure<MaterialDefinition>("Assets/Resources/Materials/"+name+".asset",m=>{m.id=id;m.displayName=title;m.description=description;m.pickupPrefab=prefab;});
        static GameObject ConsumableModel(string name,bool bottle)
        {
            const string folder="Assets/Art/Gathering";string path=folder+"/"+name+".prefab";
        ...
        {
            string path="Assets/Art/Gathering/Expedition-"+ColorUtility.ToHtmlStringRGB(color)+".mat";
