...
{
    // Editor-only mesh authoring. Runtime scenes contain ordinary saved meshes.
    internal sealed class VoxelRegionGeometry
    {
    ...
    internal sealed class VoxelRegionHeightfield
    {
        ...
        private readonly VoxelRegionSettings settings;
        public VoxelRegionHeightfield(VoxelRegionSettings settings) { this.settings=settings; }
        private float Noise(float x,float z,float scale)
        {
            float offset=(settings.seed&65535)*.137f;
            return Mathf.PerlinNoise(x*scale+offset,z*scale+offset*.731f);
        }
        public float Height(float x,float z)
        {
            float y=3+settings.relief*(.45f*Noise(x,z,.014f)+.25f*Noise(x,z,.038f));
            float radius=Mathf.Max(Mathf.Abs(x),Mathf.Abs(z));
            float mountain=Mathf.SmoothStep(0,1,Mathf.InverseLerp(settings.size*.34f,settings.size*.49f,radius));
            ...
            return Mathf.Round(y/settings.stepHeight)*settings.stepHeight;
        }
        ...
        public Color Top(float x,float z,float height)
        {
