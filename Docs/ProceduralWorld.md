...
`Assets/Data/World/ExplorationWorldSettings.asset` contiene la semilla, radios de streaming, escalas de macroformas y detalle, propiedades de pradera/bosque/highlands, montañas, densidad de encuentros y escalado. El catálogo inicial ya está enlazado en `Assets/Data/World/WorldContentCatalog.asset`. El menú `Mismo > World > Configure procedural content` permite reconstruirlo si falta; no sobrescribe un catálogo existente.
