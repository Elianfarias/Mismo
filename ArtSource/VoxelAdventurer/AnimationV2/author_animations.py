...
def body(bob=-.025,lean=0,twist=0,sway=0):
 # Upright bones have local Y pointing up; local Z points backward.
 reset();P['Hips'].location=R['Hips'].matrix_local.to_3x3().inverted()@Vector((sway,0,bob));rot('Spine',lean*.40,twist*.35,0);rot('Chest',lean*.60,twist*.65,0);rot('Head',-lean*.35,-twist*.6,0);update()
def idle(t):
...
def gait(t,run=False):
 wave=math.sin(t*math.tau);bob=(-.035-.012*math.cos(t*4*math.pi)) if not run else (-.055-.025*math.cos(t*4*math.pi))
 body(bob,6 if not run else 12,(-4 if not run else -6)*wave,.006*wave)
 for side,sign in [('L',1),('R',-1)]:
  u=(t+(0 if sign==1 else .5))%1
  # Stance occupies 60% of the walk; swing clears the ground with a bent knee.
  stance=.60 if not run else .40;stride=.38 if not run else .62
  if u<stance: y=-stride/2+stride*u/stance;z=.17;pitch=0
  else:
   v=(u-stance)/(1-stance);travel=v*v*(3-2*v);y=stride/2-stride*travel;z=.17+(.09 if not run else .20)*math.sin(v*math.pi);pitch=12*math.sin(v*math.tau)
  foot(side,(sign*.15,y,z),pitch)
 # Weapon arm swings less, free arm counterbalances the legs.
 hands(right=(-.30,-.07+(.07 if not run else .13)*wave,.88+(.00 if not run else .14)),left=(.30,-.03-(.12 if not run else .20)*wave,.84+(.00 if not run else .18)))
def jump(t):
...
def spin(t):
 body(-.06,4,0)
 axis=R['Root'].matrix_local.to_3x3().inverted()@Vector((0,0,1))
 P['Root'].rotation_quaternion=Quaternion(axis,-math.tau*t);update()
...
# Importing the weapon changes the scene rate to its FBX rate (30 fps).
# Restore the rate used to author all keys before saving/exporting the character.
...
for name,duration,_ in [spec for spec in specs if spec[0] in ('Run','Spin')]:
